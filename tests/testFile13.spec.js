...
test('test 13 - Cart Lock & Unlock & Sale', async ({}, testInfo)=> {
 await runTest(async (testInfo) => {
    ...
    await expect(window.locator('#main > app-plastic-bag > app-main-content > div > div.is-rtl.side > app-minimal-basket > div > div.items > app-minimal-basket-item:nth-child(1)')).toContainText(`X1 ${dataset[6].changeitemPrice}`);
