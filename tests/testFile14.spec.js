...
const { getHelp, startTrs, voidTrs, ageRestriction, manualBarcode, restoreMessage, addWeightMessage } = require('./cartFunctions');
...
test('test 14 - Add PersonalBag & Add Weight & Sale', async ({}, testInfo) => {
await runTest(async (testInfo) => {
    ...
    await expect(window.getByText('סה"כ לתשלום ₪53.80')).toBeVisible();
    await expect(window.getByText('תשלום₪53.80')).toBeVisible();
  ...
  }, 'test 14 - Add PersonalBag & Add Weight & Sale',testInfo);
  });
