...
await runTest(async (testInfo) => {
  ...
  await window.getByRole('contentinfo').getByText('₪0.98').click();
  ...
  await expect(window.locator('#main > app-plastic-bag > app-main-content > div > div.is-rtl.side > app-minimal-basket > div > div.items > app-minimal-basket-item:nth-child(1)')).toContainText('בצל יבש0.200 ק"ג₪0.98');
  //
   
  await expect(window.getByText('סה"כ לתשלום ₪0.98')).toBeVisible();
  await expect(window.getByText('תשלום₪0.98')).toBeVisible();
