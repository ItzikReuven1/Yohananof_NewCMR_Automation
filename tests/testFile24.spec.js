...
test('test 24 - Sales involving a large quantity of items & adding Ghost Weight , including voided items', async ({}, testInfo) => {
await runTest(async (testInfo) => {
  ...
  await sendSecurityScale(0.0);
  await window.waitForTimeout(10000);
}, 'test 24 - Sales involving a large quantity of items & adding Ghost Weight , including voided items',testInfo);
});