...
test('test 12 - Sales involving a large quantity of items, including voided items', async ({}, testInfo) => {
await runTest(async (testInfo) => {
  ...
  await expect(window.locator('#main-basket-items-container > div > div:nth-child(3)')).toContainText('Caret UpCaret Down16');
  ...
  await expect(window.locator('#main-basket-items-container > div > div:nth-child(4)')).toContainText('Caret UpCaret Down11');
  ...
  await expect(window.locator('#main-basket-items-container > div > div:nth-child(5)')).toContainText('Caret UpCaret Down11');

  await expect(window.locator('div').filter({ hasText: 'סה"כ חסכת₪0.00' }).nth(1)).toBeVisible();
  await expect(window.getByRole('button', { name: 'תשלום (50 פריטים) ₪411.20' })).toBeVisible();
  await window.getByRole('contentinfo').getByText('₪411.20').click();
  await window.waitForTimeout(3000);
  ...
  await expect(window.locator('#main > app-plastic-bag > app-main-content > div > div.is-rtl.side > app-minimal-basket > div > div.items > app-minimal-basket-item:nth-child(3)')).toContainText('X16 ₪302.40');
  ...
  await expect(window.locator('#main > app-plastic-bag > app-main-content > div > div.is-rtl.side > app-minimal-basket > div > div.items > app-minimal-basket-item:nth-child(4)')).toContainText('X11 ₪46.20');
  ...
  await expect(window.locator('#main > app-plastic-bag > app-main-content > div > div.is-rtl.side > app-minimal-basket > div > div.items > app-minimal-basket-item:nth-child(5)')).toContainText('X11 ₪31.90');
  // 
  //await expect(window.getByText('חסכון (מבצעים והנחות) ₪11.60')).toBeVisible();
  await expect(window.getByText('סה"כ לתשלום ₪411.20')).toBeVisible();
  await expect(window.getByText('תשלום₪411.20')).toBeVisible();
  await buyBags('20');
  await window.waitForTimeout(30000);
  ...
  await expect(window.locator('div').filter({ hasText: 'סה"כ חסכת₪0.00' }).nth(1)).toBeVisible();
  await expect(window.getByRole('button', { name: 'תשלום (70 פריטים) ₪413.20' })).toBeVisible();
  await window.getByRole('contentinfo').getByText('₪413.20').click();
  await window.waitForTimeout(2000);
  ...
  await sendSecurityScale(0.0);
  await window.waitForTimeout(10000);
}, 'test 12 - Sales involving a large quantity of items, including voided items',testInfo);
});